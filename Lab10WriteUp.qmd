...
Note that it doesn't matter if the sum's lower limit is $-\infty$, $0$, or any value in between. This is because the PMF $f_{X}(x)$ includes an indicator function, $I(x \in \{0, 1, ..., n\})$, that forces the PMF to equal $0$ for any $x$ outside of the specified support. To compute this computationally, we will compute the sum exclusively over the support.
...
**Solution:**  
```{r}
llaltbinom = function(x, n, p, theta){
  sum(log(daltbinom(x = x, size = n, prob = p, theta = theta)))
}
```
...
**Solution:**   
To begin, we create a function that takes in a vector of parameters as an input ($p$ = `params[1]`, $\theta$ = `params[2]`), 
